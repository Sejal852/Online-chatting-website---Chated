...
<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top header-top" style="background-color: #292b2c !important;">
  <a class="navbar-brand" href="conversations.php">Chated | <?php echo ucfirst($username) ?></a>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <div class="form-inline my-2 my-lg-0">
    	<form action="<?php echo htmlentities($_SERVER['PHP_SELF']) ?>" method="POST">
    		<button type="submit" name="logout" class="btn btn-chated-logout btn-sm" style="margin-right: 40px;">Logout</button>
    	</form>
		<div class="account-box">
			<?php if(!Image::hasProfileImage($user_id)) { ?>
				<img src="assets/avatars/default.png" alt="Profile Pic" class="rounded-img header-img" />
    		<?php } else { ?>
						...
						foreach ($usernames as $single_username) {
							if($single_username['id'] != Login::isLogged()) {
								...
								if(!Image::hasProfileImage($single_username['id'])) {
									echo '<img src="assets/avatars/default.png" alt="Profile Pic" />';
								} else {
		...
		<section class="col-sm-12 col-md-8 clearfix messages">
			...
			<div class="write-your-message">
				<form action="<?php htmlentities($_SERVER['PHP_SELF']) ?>" method="POST" id="js-sendMessage">
					<input type="text" class="input-chated" id="js-messageBody" name="message" placeholder="Write your message" style="display:none"/>
